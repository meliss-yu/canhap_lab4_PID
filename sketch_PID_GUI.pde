...
void setup(){
  ...
  //path = createShape(ELLIPSE, 0, 0, 200, 200);
  //path.setStroke(color(0));
  path = createShape(RECT, 0, 0, 0.8, 0.4);
  path.setStroke(color(0));
...
while(1==1) {
    ...
    if(haplyBoard.data_available()){
      ...
      if (yr <= 0 && xr < 0.8){
        yr = 0;
        xr = xr+0.0001;
      } else if (xr >= 0.8 && yr < 0.4){
        yr = yr+0.0001;
        xr = 0.8; 
      } else if (yr >= 0.4 && xr <= 0.8 && xr > 0){
        xr = xr-0.0001; 
        yr = 0.4; 
        print(xr, yr);
      } else if (yr <= 0.4 && xr <= 0){
 ...
 // Torques from difference in endeffector and setpoint, set gain, calculate force
    ...
    // Forces are constrained to avoid moving too fast
      ...
      //fEE.x = constrain(P*dist_X,-4,4) + constrain(I*cumerrorx,-4,4) + constrain(D*buffx,-8,8);

      
      fEE.y = constrain(P*dist_Y,-4,4) + constrain(I*cumerrory,-4,4) + constrain(D*diffy,-8,8); 
      //fEE.y = constrain(P*dist_Y,-4,4) + constrain(I*cumerrory,-4,4) + constrain(D*buffy,-8,8); 
